...
1. 属性（Attribute）：
   - 在现实世界中，要描述一个事务常常取若干特征来表示。这些特征称为属性。例如学生通过学号、姓名、性别、系别、年龄、籍贯等属性来描述。
2. 域（Domain）：
   - 每个属性的取值范围对应一个值的集合，称为该属性的域。例如，学号的域是6位整型数；姓名的域是10位字符；性别的域为{男，女}等。
3. 超键（Super Key）：
   - 超键是一组属性。能够唯一标识表中每一行记录。超键可以包含多余属性，即便不是必须的，结合起来也可以唯一标识记录。
4. 候选码（Candidate Key）：
   - 候选码是最小的超键。即从超键中移除任何属性后，它就不再能唯一标识表中的每一行记录。
5. 主键（Primary Key）：
   - 若一个关系中有多个候选码，则选定其中一个作为主键。主键也是最小的超键。一旦选定，应尽量避免更改。
6. 主属性（Prime Attribute）：
   - 包含在任何候选码中的属性称为主属性，不包含在任何候选码中的属性称为非主属性（Non-Prime Attribute）。
7. 外键（Foreign Key）：
...
当一个 `属性集合A` 完全决定另一个 `属性集合B` 时，我们说B完全函数依赖于A。记作 `A->B` 。这意味着A中的每个值都决定了B的一个唯一值。

> （学号，课程）-> 成绩。学号和课程单独拿出来都不能决定成绩。所以说 `成绩` 完全依赖于 `(学号，课程)`。

##### 3.1.2 部分函数依赖
定义：若X->Y，但Y并**不完全函数依赖**于X，则称Y对X**部分函数依赖**。

> (学号，课程) -> 学生姓名。这里`学生姓名`只函数依赖于`(学号，课程)`中的子集`学号`，此时 `学生姓名` 部分依赖于`（学号，课程）`。

##### 3.1.3 传递函数依赖

当一个属性集合A通过中间属性集合B决定另一个属性集合C时，我们说C传递函数依赖于A。记作`A->B->C`。

> 学号 -> 班级，班级 -> 班主任。这里`班主任`依赖于所在`班级`，而所在`班级`又依赖于`学号`。

#### 3.2 多值依赖

定义：
- 存在关系模式`R(U)`，X、Y、Z是U的子集，并且 Z = U-X-Y。
- 若要关系R(U)中**多值依赖** X ->-> Y成立，**当前仅当**对R(U)的任一关系r，给定的一堆`(x,z)`值，有一组`y`的值，这组值仅仅决定于`x`的值而与`z`值无关。

例1：  

`学生选课表Table_MSC(M,S,C)`中，M表示专业，S表示学生，C表示必修课。
...
`学生兴趣表Table_ISA(I,S,A)`中，I表示兴趣小组，S表示学生，A表示某兴趣小组的活动项目。

假设每个兴趣小组有多个学生，有若干活动项目，每个学生必须参加所在兴趣小组的所有活动项目，每个活动项目要求该兴趣小组的所有学生参加。
...
可以看出，每列都是不可再分的数据项，所以是1NF的。

在上述关系模式中，可以看到它的候选码是`(Sno, Pno)`，但存在`(Sname, Status)`部分函数依赖于 `(Sno, Pno)`。

`Sno`可以单独推断出`Sname`和`Status`，这就导致其存在插入异常和删除异常的问题。
...
- 定义：在2NF的基础上，**消除**非主键列之间的**传递依赖**，即任何**非主键列**都**不依赖**于其他**非主键**列。
- 目的：消除传递依赖，提高数据表的稳定性和一致性。

在满足了2NF之后，继续分析可以发现，关系Supplier中存在**传递函数依赖**。

`Sno->Status`, `Status->City`

导致其存在修改操作的不一致性问题：
- 如Status=20对应的City从天津搬到了上海，则有可能导致一些Status=20的数据被修改，而另一些不被修改，导致数据产生不一致性。
  
为了解决这个问题，就需要3NF。将Supplier进一步分解：
...
- 定义：在3NF的基础上，要求任何**非主键**列**不依赖**于主键的任何**超键**。
- 目的：消除主键**多值依赖**产生的问题，确保表的完全依赖关系。
  
这意味着：
- 任何决定因素（Determinant）必须是候选键：在 BCNF 中，如果一个列 A 决定另一个列 B（即 A → B），那么 A 必须是一个候选键（即最小的超键）。

假设有一个表 `Employee`，包含以下列：
...
在这个表中，`EmployeeID` 是主键，`(EmployeeID, DepartmentID)` 组成一个超键。

1. 3NF 分析：
   - `DepartmentID` 和 `DepartmentName` 之间存在传递依赖：`EmployeeID → DepartmentID` 和`DepartmentID → DepartmentName` 。

为了满足 3NF，可以将 `DepartmentID` 和 `DepartmentName` 分离到一个新的表 `Department` 中。

2. BCNF 分析：
   - 即使在 3NF 中已经消除了传递依赖，但在 `Employee` 表中，`DepartmentName` 仍然依赖于 `DepartmentID`，而 `DepartmentID` 是一个超键的一部分。
  
为了满足 BCNF，需要进一步确保 `DepartmentName` 不依赖于 `DepartmentID`。这可以通过将 DepartmentName 移动到 Department 表中来实现。
...
但是由于学生编号与选课之间，学生编号与爱好之间分别存在多值依赖，且选课和爱好之间没有任何关系，所以这里存在数据冗余。每多一个选课都要添加2条记录（篮球和羽毛球）。

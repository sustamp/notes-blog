...
|部署图|软件和硬件组件之间的**物理关系**以及处理节点的组件分布情况|
...
1. 在定义上：状态图描述的是一个对象所有可能的状态，以及因为事件行为的发生而引起的状态变化；活动图描述的是对象的活动顺序所遵循的行为规则。
2. 状态图着重表现行为的结果；活动图着重表现行为的动作。
...
主要描述的是系统或对象的动态行为，强调活动（动作）的流程，展示为一步步的**控制流**和**数据流**，包括顺序、**并发**、分支和同步等逻辑。
...
###### 五种视图（Δ）
对于同一个系统，**不同人员**所关心的内容不一样，按照图本身的特点，可以把图形划分为5种视图，分别是：
- 用例视图
- 逻辑视图
- 进程视图
- 实现视图
...
|视图|要点描述|图形表示|关注成员|
|----|----|----|----|
|用例视图|描述系统的功能需求|用例图|客户、分析者、设计者、开发和测试者|
|逻辑视图|描述如何实现系统内部的功能，系统的静态结构和信息交互的动态协作|类图和对象图、状态图、时序图和协作图、活动图|系统分析和设计人员|
|进程视图|||开发和系统集成人员|
|实现视图|||设计人员、开发和测试|
|部署视图|||开发、测试、运维人员|

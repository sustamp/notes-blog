...
> CORBA， Common Object Request Broker Architecture，公共请求代理架构。

CORBA的主要对象和部件：
- **伺服对象（Servant）**：CORBA对象的真正实现，负责完成客户端的请求
- **对象适配器（Object Adapter）**：屏蔽ORB内核的实现细节，为服务器对象提供抽象接口，以实用ORB的内部功能。
- **对象请求代理（Object Request Broker）**：解释调用并负责查找实现该请求的对象，将参数传给找到的对象，调用方法并返回结果。客户方不需要了解服务对象的位置、通信方式、实现、激活或存储机制。

...
|网络层|分组传输和路由选择|通过网络连接交换传输层实体发出的数据，解决路由选择、网络拥塞、异构网络互连的问题。服务访问点为逻辑地址（网络地址）。**协议有：IP、IPX等**|
|数据链路层|传送以帧为单位的信息|建立、维持和释放网络实体之间的数据链路（*网络层和物理层的中介*），把流量控制和差错控制合并在一起。分为MAC（媒介访问层）和LLC（逻辑链路层）两个子层。服务访问点为物理地址（MAC地址）。协议有：IEEE 802.3/2、HDLC、PPP、ATM等|
|物理层|二进制位传输|通过一系列协议定义了通信设备的机械的、电气的、功能的、规程的特征。RS232、V.35、RJ-45、FDDI等|
...
###### 协议集比较
OSI是7层模型，TCP/IP重新衡量各层的作用，又分化为4层模型。其二者间用到的协议是不同的。

传输层有2个最重要的协议：TCP、UDP。

但使用TCP协议的协议太多了，那么我们主要记录下 **基于UDP的协议** 来区分：
- VIOP、SNMP、TFTP、DNS、DHCP
...
|---|---|---|---|
|应用层|...|...|...|
||HTTP|80|超文本传输协议（Hyper Text Transfer Protocol），建立在TCP之上|
||HTTPS|443|超文本传输安全协议（HTTP-Secure）。配置服务器时，确保 443 端口开放且 SSL 证书有效|
||FTP|数据端口20，控制端口21|文件传输协议（File Transfer Protocol）|
||TFTP|69/UDP|普通文件传输协议（Trivial File Transfer Protocol）|
...
||DNS|53/UDP|域名系统（Domain Name System），域名和IP地址的映射|
||DHCP|67/UDP|动态主机配置协议，为终端设备提供IP地址、子网掩码、网关等配置信息，是典型的应用层服务|
|传输层|...|...|...|
||TCP||可靠的文件传输协议，面向连接|
||UDP||不可靠的文件传输协议，不面向连接|
|网际层（网络层）|...|...|...|
||ICMP||网络控制协议，用于传递网络状态信息（如ping、traceroute）|
||IGMP||管理IP组播（Multicast）组成员关系|
|网络接口层|...|...|...|
||ARP||地址解析协议。解决IP地址到MAC地址的映射，直接与物理网络（如以太网）交互，不依赖IP协议。|
||RARP||功能与ARP相反（MAC地址→IP地址），同样直接操作数据链路层（OSI模型）层帧。已淘汰，被DHCP取代|

##### DHCP
DHCP用于自动为客户端分配IP地址、子网掩码、默认网关、DNS服务器等网络配置信息，避免手动配置的麻烦。

广泛用于家庭网络、企业网络和公共Wi-Fi。

###### 要点
1. 一个园区网络可以有多个DHCP服务器（*1对多*）。1个DHCP服务器可以提供多个网段的IP（*本网段/跨网段*）。
2. 所有IP由DHCP集中管理。分配方式有三种：
   - 固定分配（管理员分配）
   - 动态分配（IP地址有效期8天）
   - 自动分配（长期有效的IP地址）

###### DHCP工作原理
DHCP通过 4个核心报文 完成IP分配，简称 **DORA**(*Discover, Offer, Request, Acknowledgement*)，主要工作原理如下：
1. **DHCP Discover**（客户端-服务器）
   - 客户端首次启动时，广播发送 `DHCP Discover` 数据包，表达IP租约的请求，寻找可用的DHCP服务器。
2. **DHCP Offer**（服务器-客户端）
   - 服务器响应 `DHCP DISCOVER`，提供可用的 IP地址及其他配置（子网掩码、网关、DNS）。
   - 仍然是广播，因为客户端没有IP。
3. **DHCP Request**（客户端-服务器）
   - 客户端选择第一个收到（*当有多个DHCP服务器响应时*）的 `DHCP OFFER`，广播 REQUEST 数据包以确认租用该IP。
   - 仍然是广播，以告知其他DHCP服务器已经选好了。
4. **DHCP Ack**（服务器-客户端）
   - 服务器发送 `DHCP ACK`，确认IP分配成功，客户端正式使用该IP
   - 如果IP不可用（被其他设备占用），服务器可能回复 **DHCP NAK**（拒绝）。
5. 冲突检测
   - 客户端收到 `DHCP ACK` 后，发送 **ARP请求** 检查该IP是否已被使用。
   - 如果收到ARP响应，则IP冲突，客户端发送 `DHCP Decline` 拒绝该IP。
   - 服务器将该IP标记为冲突，不再分配。
   - 客户端重新发起 **DORA过程**（获取新IP）。
6. 续租：DHCP分配的IP是 临时租用 的，客户端需在到期前续租，否则IP会被回收。
   - T1（50%租期）：租期过半时，客户端向原服务器申请续租（单播 `DHCP REQUEST`）
   - T2（87.5%租期）：此时如果原服务器还没响应，客户端则 广播 `DHCP REQUEST` 寻找其他服务器。
   - T3（到期未续租）：IP被回收，客户端必须会重新发起 DORA过程 获取新IP。
7. DHCP Release：客户端主动释放IP。

#### DNS
##### 解析顺序
(1) 浏览器输入域名

HOSTS文件 -> 本地DNS缓存 -> 本地DNS服务器 -> 根域名服务器 -> 顶级域名服务器 -> 权限域名服务器
...
##### 迭代查询与递归查询
- 迭代查询：
  - 服务器收到一次查询返回一次结果，这个结果并不一定是最终答案。
  - (*问路，A答：不知道，你问B*)
- 递归查询：
  - 服务器带着问题一个个查下去，知道得到明确答案。
  - （*问路，B答：我问下C，C问下D，知道答案返回*）
...
|进程视图（*过程视图*）|以可执行线程和进程作为活动类的建模，它是逻辑视图的**一次执行实例**。描述系统的**运行时行为**，重点关注并发、线程、进程间的通信与同步|活动图、状态图、协作图、时序图|开发和系统集成人员（架构设计阶段）|
...
系统工程是一种从**整体**出发、从**系统观念**出发的科学决策方法，以求整体最优。所以它的特点就有：
- 整体性
- 综合性
- 协调性
- 科学性
...
> 又是7个步骤。记忆：为同学做个NBA比赛分数统计与预测
> - **探索**：看看他现在看的网站都有些啥东西，我要关注的分数在什么区域，如何抓取，抓取后的数据要怎么处理和试用？
> - **概念**：这里的分数有什么初指、综指的，是啥意思？他拿到数据之后要做什么，怎么叫预测？如何实现？
> - 开发：搞清楚概念和需求后，可以动手开发了
> - **生产**：开发好了之后，看下能不能用，数据是否能正确抓取，功能是否满足要求
> - 报障：可以用了，那就要保证一直能用
> - 退役：最初的应用只能放到PC机上，并且要电脑一直开着，有没有别的办法能用手机也看到，如果能实现，那之前做的东西就能退役了。
...
#### 段式存储
段式存储相当于一个家庭分配一个房子。此时：
- 逻辑地址 = 段号 + 段内地址（*偏移量*）
  - 段号：看段表中的段号
  - 段内地址：看段表中的段长。
- 物理地址 = 基地址 + 偏移量
  - 逻辑地址能转换为物理地址的前提是段长不越界，否则无法转换。

假设某系统采用段式存储管理方法，进程P的段表如下：
...
此时，若有（0, 810）和(2, 210)逻辑地址则不能转换为对应的物理地址，因为：逻辑地址到物理地址转换时越界。

#### 段页式存储
段式 + 页式的结合体，先将用户程序分成若干段，再把每个段分成若个页。她的特点如下：
- **页大小相同**：每个页的大小相同（*页大小一致*）。
  - 假设页内地址的位数是：0~12，则页大小为：2^13 = 2^3 * 1024（即8K）。
- **页个数可能不同**：段内的页个数不同。
  - 假设页号位数为：13~23，则页个数最多为：2^11 = 2048。
- **段个数可能不同**：每个段的大小不同。
  - 假设段号位数为：24~31，则段个数最多为：2^8 = 256。

地址结构：段号 + 段内页号 + 页内地址。
- 逻辑地址 = 段号 + 页号 + 页内地址。
- 物理地址 = 块的地址 + 页内地址。
  - 块地址：通过段表找到页表的起始地址，看页号在页表中的位置找到对应的物理块地址。


优点：空间浪费小，存储共享与保护容易，能动态链接。
缺点：由于管理软件的增加，复杂性和开销会大起来，执行速度大大下降。

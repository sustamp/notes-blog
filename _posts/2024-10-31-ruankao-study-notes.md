...
|2|带宽|1.信号频道带宽，单位：Hz; 2.网络线路单位时间内可以传输的最大数据量（**水管容量**，单位时间内通过管道的最大水量），单位：b/s||
|3|吞吐量|单位时间内网络（信道、接口）实际传输的数据量（**单位时间内从管道中流出的水**），受带宽或网络额定速率影响，吞吐量永远不会超过带宽||
...
#### 通信技术
计算机网络是利用通信技术将数据从一个节点传送到另一个节点的。通信技术是计算机网络的基础。

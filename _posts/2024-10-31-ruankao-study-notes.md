...
|处理能力|处理数字与符号|处理数字|
|处理问题的种类|准结构或非结构，可处理不确定的知识，适用于特定领域|多为结构性，处理确定的知识|

专家系统的组成：
1. **综合数据库**（*数据级*）：描述问题状态的综合库。
2. **知识库**（*知识库级*）：存放启发式经验的知识。
3. **推理机**（*控制级*）：对知识库的只是进行推理。

![DSS结构](https://sustamp.github.io/assets/pictures/ruankao/Expert-Structure.png)

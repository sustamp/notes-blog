...
> 如果各段流水的操作时间不同，则流水线的吞吐率是 **最长流水段操作时间**的**倒数**
...
如果这3个进程都要求使用2个互斥资源R，那么系统不产生死锁的最少的R 资源数为（  ）个。
```
...
假设进程数为`m`，每个进程需要`k`个资源，总资源数为`n`，**避免死锁的最少资源数**可以通过如下公式计算：
- `n` >= `m`(`k` - 1) + 1
...
> - RISC: Reduced Instruction Set Computer，精简指令集计算机。核心思想：**让硬件做简单的事情，复杂的交给软件**。
...
1. 半双工/全双工，适合长距离传输
2. 串行总线按bit（位）发送和接收
3. 串口通信时两个端口的参数（波特率、数据位、停止和奇偶校验）需要匹配，使用中不可改变。
4. 串行总线的数据正确性依赖**校验**和**重传**
5. 串行总线的数据发送和接受可以使用**查询**和**中断**两种方式。
...
##### Web服务器的性能评估
上面提到过，Web服务器的性能指标有 最大并发连接数（或*最大并发用户数*）、响应延迟（*接口响应时间*）、吞吐量。
